...
私は Unityを使ったVRCアバター向け衣装作成販売と、趣味で簡単なギミックを数個作ったくらいしか知識がないエンジニア初学者です。さらに、Web開発の実務経験はありません。
...
- 承認あるまでのコードの書き換え、コマンドの実行は固く禁ずる。
  - 承認は必ず取ること。
- **コーディング訓練規定 (Strict Training Mode)**
  - 訓練校では満足なコーディング経験が得られなかったため、自身のスキル向上（タイピング速度・精度）を目的として、**原則としてコードのコピペを禁止し、手動入力を徹底する。**
  - AIは「コピペ用コード」ではなく「写経用のお手本コード」として提示すること。
  - **コード内コメントの義務化:**
    1.  **Vanilla JS/Web基礎比喩:** Unity/C#の言及を廃止し、「素のJavaScript (Vanilla JS)」や「DOM操作」と比較して、Reactのメリットや挙動を説明する。
    2.  **セキュリティ知識:** その実装におけるセキュリティ上の注意点（なぜ安全か、どんなリスクがあるか、例外的な許可・禁止事項）を詳細に記述する。
- 常に敬語であること。
