...
- 出力されるすべてのImplementation Planは、作業手順ログとして残すため、その時の日付のフォルダが階層内にない場合にのみDocs\Work_Log下に{YY'MM-DD}の名前でフォルダを作成の後、以下のルールに従って出力する。
  - 出力されたImplementation Planは、対応する日付のフォルダ内(YY'MM-DD)に{Num_内容.md}のタイトル形式と拡張子で残し、同日の出力内容は同日のファイルに追記していく形で進行する。(YYは20YYの末尾2桁とする。MMは01-12の数字とする。DDは01-31の数字とする。Numは00から順番に進める。)
  - 追記時は既存の内容を消さず、新しい手順や実行結果(Status: [x] Done等)を下に追加すること。
  - その日の総まとめを{YY'MM-DD_DailyReport.md}として出力すること。
  - また、出力されたImplementation PlanとDailyReportを1週間毎にまとめて、Docs\01_Logs\00_Work_Log\00_Weekly_Log\YY'MM\{YY'MM_{序数}Week.md}の名前で残し、書き出しに対象期間を{MM-DD~MM-DD}の形式で記載する。(序数は1-5とする。)
  尚、一週間の始まりは月曜日を起点とする。
